...
<!-- ============================================================== -->
   ...
   <xsl:template match="iso:include-set">
       <xsl:variable name="getrules">
           <iso:fragment><xsl:apply-templates select="iso:include" mode="getrulestogether"/></iso:fragment>
       </xsl:variable>
      
       <xsl:variable name="compactedrules">
       <iso:schema    
...
<iso:ns prefix="init" uri="urn:oasis:names:tc:SAML:profiles:SSO:request-init"/>
           <iso:pattern id="{generate-id()}"><xsl:apply-templates select="exsl:node-set($getrules)/iso:fragment/*" mode="compactrules"/></iso:pattern>
     </iso:schema>
     </xsl:variable>

<xsl:variable name="expandedrules">
<iso:fragment>
 <xsl:apply-templates select="exsl:node-set($compactedrules)//iso:rule"/>
</iso:fragment>
</xsl:variable>
    <axsl:template match="/" name="bryansgeneratedtemplate">
      <xsl:apply-templates select="exsl:node-set($expandedrules)//xsl:template[@mode]" mode="bryansxslgenerator"/>
    
     </axsl:template>
     <xsl:apply-templates select="exsl:node-set($expandedrules)//xsl:template[@mode]" mode="removetextnodes"/>
  <xsl:copy-of select="exsl:node-set($expandedrules)/iso:fragment/*"/>   
</xsl:template>
   
   <xsl:template match="xsl:template" mode="removetextnodes">
   <xsl:variable name="mode" select="@mode"/>
   <xsl:choose>
   <xsl:when test="preceding::xsl:template[@mode=$mode]"/>
   <xsl:otherwise>
     <axsl:template match="text()" mode="{$mode}" />
	</xsl:otherwise>
	</xsl:choose>
	</xsl:template>
   <xsl:template match="xsl:template" mode="bryansxslgenerator">
   <xsl:variable name="mode" select="@mode"/>
   <xsl:choose>
   <xsl:when test="preceding::xsl:template[@mode=$mode]"/>
   <xsl:otherwise>
   ...
   <xsl:template match="iso:*" mode="compactrules">
      <xsl:copy><xsl:copy-of select="@*"/><xsl:apply-templates mode="compactrules"/></xsl:copy>
   </xsl:template>
   <xsl:template match="iso:assert | iso:report" priority="10" mode="compactrules"/>
   

  
   <xsl:template match="iso:rule" mode="compactrules">
     <xsl:variable name="context" select="@context"/>
     <xsl:choose>
      <xsl:when test="preceding::iso:rule[@context = $context]"/>
      <xsl:otherwise>
      <xsl:copy><xsl:copy-of select="@*"/>
        <xsl:copy-of select="//iso:rule[@context=$context]/*"/>
      </xsl:copy>
      </xsl:otherwise>
      </xsl:choose>
   </xsl:template>
   <xsl:template match="iso:include" mode="getrulestogether">
   <xsl:variable name="document-uri" select="substring-before(concat(@href,'#'), '#')"/>
       <xsl:variable name="fragment-id" select="substring-after(@href, '#')"/>
       ...
       <xsl:choose> 
          ...
          <xsl:when test="string-length( $fragment-id ) &gt; 0">
              ...
              <xsl:copy-of select=" $theFragment_1"/>
		   </xsl:when>
		   ...
		   <xsl:otherwise>
              ...
              <xsl:variable name="theFragment_2" select="$theDocument_2/iso:*" />
              <xsl:choose>
              <xsl:when test="$theDocument_2/iso:fragment">
       		    <xsl:copy-of select="$theFragment_2/iso:fragment/iso:*"/>
       		 </xsl:when>
       		 <xsl:otherwise>
       		    <xsl:copy-of select="$theFragment_2/iso:*"/>
       		 </xsl:otherwise>
       		 </xsl:choose>
       	   </xsl:otherwise>
       </xsl:choose>
