...
Validating entityID <axsl:text/><axsl:value-of select="@entityID"/><axsl:text/>
     XPATH: <axsl:call-template name="xpathgetter"/> validation rule: (@entityID)</axsl:message></axsl:if><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M0"/></axsl:template><axsl:template xmlns:axsl="http://www.w3.org/1999/XSL/Transform" match="text()" priority="-1" mode="M0"/><axsl:template xmlns:axsl="http://www.w3.org/1999/XSL/Transform" match="@*|node()" priority="-2" mode="M0">
