...
Warning (33): IDPSSODescriptor should contain alg:DigestMethod
     XPATH: <axsl:call-template name="xpathgetter"/> validation rule: (md:Extensions/alg:DigestMethod)</axsl:message></axsl:otherwise></axsl:choose><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M0"/></axsl:template>
