...
<!--SCHEMATRON PATTERNS-->
<axsl:template match="/" name="bryansgeneratedtemplate"><axsl:apply-templates mode="M10"/></axsl:template><axsl:template match="text()" mode="M10"/><axsl:template match="md:EntityDescriptor" priority="1002" mode="M10">

		<!--REPORT -->
<axsl:if test="@entityID"><axsl:message>
Info: Validating entityID <axsl:text/><axsl:value-of select="@entityID"/><axsl:text/>
     XPATH: <axsl:call-template name="xpathgetter"/> validation rule: (@entityID)</axsl:message></axsl:if><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M10"/></axsl:template><axsl:template match="//md:IDPSSODescriptor" priority="1001" mode="M10">
...
Error (04): Each IDPSSODescriptor must contain a signing key as X509Certificate (child element of X509Data)         
         XPATH: <axsl:call-template name="xpathgetter"/> validation rule: (md:KeyDescriptor[@use='signing' or not(@use)]/ds:KeyInfo/ds:X509Data/ds:X509Certificate)</axsl:message></axsl:otherwise></axsl:choose><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M10"/></axsl:template><axsl:template match="//md:SPSSODescriptor" priority="1000" mode="M10">
...
Error (04): Each SPSSODescriptor must contain a signing key as X509Certificate (child element of X509Data)
         XPATH: <axsl:call-template name="xpathgetter"/> validation rule: (descendant::ds:X509Data/ds:X509Certificate)</axsl:message></axsl:otherwise></axsl:choose><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M10"/></axsl:template></axsl:stylesheet>
