...
Info: Validating entityID <axsl:text/><axsl:value-of select="@entityID"/><axsl:text/>
     XPATH: <axsl:call-template name="xpathgetter"/> validation rule: (@entityID)</axsl:message></axsl:if><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M10"/></axsl:template><axsl:template match="//md:IDPSSODescriptor" priority="1000" mode="M10">
...
Error (09): IDPSSODescriptor must contain a SingleSignOnService with Binding="urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect“
     XPATH: <axsl:call-template name="xpathgetter"/> validation rule: (md:SingleSignOnService[normalize-space(@Binding)='urn:oasis:names:tc:SAML:2.0:bindings:HTTP-Redirect'])</axsl:message></axsl:otherwise></axsl:choose><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M10"/></axsl:template></axsl:stylesheet>
