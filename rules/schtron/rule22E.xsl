...
Info: Validating entityID <axsl:text/><axsl:value-of select="@entityID"/><axsl:text/>
     XPATH: <axsl:call-template name="xpathgetter"/> validation rule: (@entityID)</axsl:message></axsl:if><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M10"/></axsl:template><axsl:template match="mdui:UIInfo" priority="1000" mode="M10">
...
Error (22c): UIInfo must include a non-empty Logo URL
     XPATH: <axsl:call-template name="xpathgetter"/> validation rule: (mdui:Logo[string-length() &gt; 0])</axsl:message></axsl:otherwise></axsl:choose><axsl:apply-templates select="*|comment()|processing-instruction()" mode="M10"/></axsl:template></axsl:stylesheet>
