...
<axsl:template match="//md:SPSSODescriptor" priority="1000" mode="M0">
		...
		<!--REPORT -->
<axsl:if test="not(md:Extensions) or                        md:Extensions[not(idpdisc:DiscoveryResponse)]"><axsl:message>"rule17W": { "Severity": "Warning",
         "Message": "SPSSODescriptor should include Extensions/DiscoveryResponse",
             "Context": "<axsl:call-template name="xpathgetter"/>",
             "Rule": "not(md:Extensions) or md:Extensions[not(idpdisc:DiscoveryResponse)]"}
