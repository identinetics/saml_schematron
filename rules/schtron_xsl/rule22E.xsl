...
<!--PATTERN Rule22_UUInfo-->


	<!--RULE -->
<axsl:template match="mdui:UIInfo/mdui:DisplayName" priority="1000" mode="M0">

		<!--ASSERT -->
<axsl:choose><axsl:when test="string-length(normalize-space(text())) &gt; 0"/><axsl:otherwise><axsl:message>"rule22E": { "Severity": "Error",
         "Message": "UIInfo must include a non-empty DisplayName",
             "Context": "<axsl:call-template name="xpathgetter"/>",
             "Rule": "string-length(normalize-space(text())) &gt; 0"}
