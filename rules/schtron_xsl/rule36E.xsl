...
<!--PATTERN Rule36_SSOServ_HTTPPOST-->
...
<axsl:template match="md:IDPSSODescriptor" priority="1000" mode="M0">
		...
		<!--ASSERT -->
<axsl:choose><axsl:when test="md:SingleSignOnService[normalize-space(@Binding)='urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST']"/><axsl:otherwise><axsl:message>"rule36E": { "Severity": "Error",
         "Message": "IDPSSODescriptor must contain a SingleSignOnService with Binding=\"urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST\"",
             ...
             "Rule": "md:SingleSignOnService[normalize-space(@Binding)='urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST']"}
